...
export async function POST(request: Request) {
  try {
    ...
    const order = data as { id: string; orderNumber: string; subtotal: number }; const { data: lines } = await supabase.from("order_items").select("product_name_snapshot, variant_snapshot, quantity, unit_price, subtotal").eq("order_id", order.id).order("created_at");
    const { data: settings } = await supabase.from("store_settings").select("whatsapp_number").eq("id",1).single(); const whatsapp = settings?.whatsapp_number || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER;
    if (!whatsapp) return NextResponse.json({ error: "El canal de WhatsApp aún no está configurado. Tu pedido quedó guardado; contacta a MJ Vault para continuarlo.", orderNumber: order.orderNumber }, { status: 503 });
    return NextResponse.json({ ...order, items: lines ?? [], whatsapp: String(whatsapp).replace(/\D/g, "") });
  } catch (error) { console.error("Order creation failed", error instanceof Error ? error.message : "unknown"); return NextResponse.json({ error: "No pudimos procesar el pedido en este momento." }, { status: 500 }); }
}
